...
Optional effort argument: `$1`

- Valid effort values: `low`, `medium`, `high`, `xhigh`, `max`.
- If `$1` is one of the valid values, perform this review at **$1** effort: apply that level of rigor and depth, and tell each of the three review agents to do a `$1`-effort pass.
- If `$1` is provided but is not recognized, say you are ignoring it, then use the session's current/default effort.
- If no effort is provided, use the session's current/default effort.
